...
found:
  ...
  //Tercera Parte
  //Inicializamos con prioridad dos, la mas alta
  p->priority = NPRIO - 1;
  //Inicializamos en cero, porque no fue schedulada todavia
  p->scheduled = 0;
...
procdump(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    printf("\n");
    printf("priority = %d\t scheduler = %d", p->priority, p->scheduled);
    printf("\n");
  }
}
